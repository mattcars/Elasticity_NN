...
'''
def assemble_hybrid(nodes, elements, utype, qtype, GP, P, Pn, Pe, mu, lam):
...
'''
def assemble(nodes, elements, etype, GP, P, Pn, Pe, E=0, v=0):
    # Global matrices
    nf = nodes.shape[0]*2
    K = np.zeros((nf, nf))
    F = np.zeros(nf)
    ...
    for i, e in enumerate(elements):
        ...
        # Stiffness Matrix
        for gp in GP:
            Ke = stiffness(xe, etype, gp[0], gp[1], gp[2], E=E[i], v = v[i])
            K[e_in] += Ke
